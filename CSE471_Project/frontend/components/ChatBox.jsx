...
export default function ChatBox({ conversationId, receiverId, receiverName, serviceContext, onClose }) {
  ...
  return (
    <div className="fixed bottom-4 right-4 w-96 h-96 bg-white border-2 border-blue-200 rounded-xl shadow-2xl flex flex-col overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-t-xl">
        <div className="flex items-center space-x-3">
          <div className={`w-3 h-3 rounded-full ${otherUserOnline ? 'bg-green-300 animate-pulse' : 'bg-gray-300'}`}></div>
          <div>
            <h3 className="font-bold text-sm">{receiverName || 'Chat'}</h3>
        ...
        <div className="flex items-center space-x-3">
          <div className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-300' : 'bg-red-300'}`} title={isConnected ? 'Connected' : 'Disconnected'}></div>
          <button onClick={onClose} className="text-white hover:bg-blue-700 rounded-full p-1 transition-colors">✕</button>
        </div>
      </div>
      ...
      {serviceContext && (
        <div className="px-4 py-3 bg-gradient-to-r from-amber-50 to-orange-50 border-b-2 border-amber-200">
          <p className="text-xs font-semibold text-amber-900 flex items-center">
            <span className="mr-2">💼</span>Service Inquiry: <span className="font-bold ml-1">{serviceContext.serviceTitle}</span>
          </p>
        </div>
      )}
      ...
      {/* Input */}
      <div className="p-4 border-t-2 border-blue-100 bg-gradient-to-t from-blue-50 to-white rounded-b-xl">
        <div className="flex gap-2 items-end">
          <input
            ...
            className="flex-1 px-4 py-2 border-2 border-blue-200 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent bg-white text-sm"
          />
          <input
            type="file"
            ref={fileInputRef}
          ...
          <button
            ...
            className="px-3 py-2 bg-gray-200 hover:bg-gray-300 rounded-full transition-colors"
            title="Attach files"
          >
            📎
          </button>
          <button
            ...
            className="px-4 py-2 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors font-medium"
            title="Send message"
          >
            ➤
          </button>
